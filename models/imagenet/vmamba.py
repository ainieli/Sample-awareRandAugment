# Code adapted from https://github.com/MzeroMiko/VMamba/blob/main/classification/models/vmamba.py
import os
import time
import math
import copy
...
from models.csm_triton import cross_scan_fn, cross_merge_fn
from models.csms6s import selective_scan_fn, selective_scan_flop_jit
from models.mamba2.ssd_minimal import selective_scan_chunk_fn
...
class VMamba(torch.nn.Module):

    def __init__(self, init_c=3, num_classes=1000, droppath=0.2,
                 aug_depth=2,
                 resolution=224, augment_space='RA', p_min_t=0.2, p_max_t=0.8,
                 norm_mean=[0.485, 0.456, 0.406], norm_std=[0.229, 0.224, 0.225]):
        super(VMamba, self).__init__()

        # only support tiny_s1l8 now
        channel_first = True
        self.model = VSSM(
            depths=[2, 2, 8, 2], dims=96, drop_path_rate=droppath,
            patch_size=4, in_chans=init_c, num_classes=num_classes,
            ssm_d_state=1, ssm_ratio=1.0, ssm_dt_rank="auto", ssm_act_layer="silu",
            ssm_conv=3, ssm_conv_bias=False, ssm_drop_rate=0.0,
            ssm_init="v0", forward_type="v05_noz",
            mlp_ratio=4.0, mlp_act_layer="gelu", mlp_drop_rate=0.0, gmlp=False,
            patch_norm=True, norm_layer=("ln2d" if channel_first else "ln"),
            downsample_version="v3", patchembed_version="v2",
            use_checkpoint=False, posembed=False, resolution=resolution,
        )
